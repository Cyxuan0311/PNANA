...
std::string EventParser::parseCtrlKey(const ftxui::Event& event) const {
    ...
    if (event == ftxui::Event::CtrlA) return "ctrl_a";
    if (event == ftxui::Event::CtrlB) return "ctrl_b";
    if (event == ftxui::Event::CtrlC) return "ctrl_c";
    if (event == ftxui::Event::CtrlD) return "ctrl_d";
    if (event == ftxui::Event::CtrlE) return "ctrl_e";
    ...
    if (event == ftxui::Event::CtrlL) {
        if (isCtrlShift(event)) return "ctrl_shift_l";
        return "ctrl_l";
    }
    if (event == ftxui::Event::CtrlM) return "ctrl_m";  // Enter 的别名
    if (event == ftxui::Event::CtrlN) return "ctrl_n";
    if (event == ftxui::Event::CtrlO) return "ctrl_o";
    if (event == ftxui::Event::CtrlP) return "ctrl_p";
    if (event == ftxui::Event::CtrlQ) return "ctrl_q";
    if (event == ftxui::Event::CtrlR) return "ctrl_r";
    if (event == ftxui::Event::CtrlS) return "ctrl_s";
    if (event == ftxui::Event::CtrlT) return "ctrl_t";
    if (event == ftxui::Event::CtrlU) return "ctrl_u";
    if (event == ftxui::Event::CtrlV) return "ctrl_v";
    if (event == ftxui::Event::CtrlW) return "ctrl_w";
    if (event == ftxui::Event::CtrlX) return "ctrl_x";
    if (event == ftxui::Event::CtrlY) return "ctrl_y";
    if (event == ftxui::Event::CtrlZ) {
        if (isCtrlShift(event)) return "ctrl_shift_z";
        return "ctrl_z";
    }
...
std::string EventParser::parseFunctionKey(const ftxui::Event& event) const {
    ...
    if (event == ftxui::Event::F3) {
        if (isShift(event)) return "shift_f3";
        return "f3";
    }
...
std::string EventParser::parseNavigationKey(const ftxui::Event& event) const {
    ...
    if (event == ftxui::Event::Home) {
        if (mods.ctrl) return "ctrl_home";
        return "home";
    }
    if (event == ftxui::Event::End) {
        if (mods.ctrl) return "ctrl_end";
        return "end";
    }
    if (event == ftxui::Event::PageUp) {
        if (mods.ctrl) return "ctrl_pageup";
        return "pageup";
    }
    if (event == ftxui::Event::PageDown) {
        if (mods.ctrl) return "ctrl_pagedown";
        return "pagedown";
    }
    return "";
}
...
std::string EventParser::parseArrowKey(const ftxui::Event& event) const {
    ...
    // Shift+方向键（用于选择）
    if (event == ftxui::Event::ArrowUpCtrl) return "shift_arrow_up";
    if (event == ftxui::Event::ArrowDownCtrl) return "shift_arrow_down";
    if (event == ftxui::Event::ArrowLeftCtrl) return "shift_arrow_left";
    if (event == ftxui::Event::ArrowRightCtrl) return "shift_arrow_right";
...
std::string EventParser::parseSpecialKey(const ftxui::Event& event) const {
    ...
    if (event == ftxui::Event::Backspace) {
        if (isCtrl(event)) return "ctrl_backspace";
        return "backspace";
    }
    if (event == ftxui::Event::Delete) {
        if (isCtrl(event)) return "ctrl_delete";
        return "delete";
    }
    ...
    if (event == ftxui::Event::Tab) {
        if (isAlt(event)) {
            auto mods = parseModifiers(event);
            if (mods.shift) return "alt_shift_tab";
            return "alt_tab";
        }
        return "tab";
    }
    if (event == ftxui::Event::TabReverse) return "shift_tab";
