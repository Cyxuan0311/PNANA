...
#endif
#include "features/SyntaxHighlighter/makefile_syntax_constants.h"
#include <algorithm>
#include <cctype>
#include <cstring>
...
std::vector<Token> SyntaxHighlighter::tokenize(const std::string& line) {
    ...
    } else if (current_file_type_ == "lisp" || current_file_type_ == "cl" ||
               current_file_type_ == "commonlisp" || current_file_type_ == "scheme" ||
               current_file_type_ == "scm") {
...
std::vector<Token> SyntaxHighlighter::tokenizeMakefile(const std::string& line) {
    ...
    while (i < line.length()) {
        ...
        if (line[i] == '$') {
            ...
            if (i < line.length()) {
                if (line[i] == '(' || line[i] == '{') {
                    char end_char = (line[i] == '(') ? ')' : '}';
                    i++;
                    int paren_depth = 1;

                    // 检查是否是函数调用（基于tree-sitter-make的语法）
                    bool is_function = false;
                    size_t func_check_start = i;
                    std::string potential_func;

                    // 收集函数名候选
                    while (func_check_start < line.length() &&
                           (std::isalnum(line[func_check_start]) || line[func_check_start] == '_' ||
                            line[func_check_start] == '-')) {
                        potential_func += line[func_check_start];
                        func_check_start++;
                    }

                    // 检查是否是已知函数名（使用makefile语法常量）
                    if (isMakefileFunction(potential_func)) {
                        is_function = true;
                    }

                    while (i < line.length() && paren_depth > 0) {
                        if (line[i] == end_char) {
                            paren_depth--;
                            if (paren_depth == 0) {
                                i++;
                        ...
                        } else if (line[i] == (end_char == ')' ? '(' : '{')) {
                            paren_depth++;
                        }
                        i++;
                    }

                    TokenType var_type = is_function ? TokenType::FUNCTION : TokenType::TYPE;
                    tokens.push_back({line.substr(start, i - start), var_type, start, i});
                    continue;
                } else {
                    // 自动变量：$@, $%, $<, $?, $^, $+, $* 等
                    // 基于tree-sitter-make的自动变量定义
                    if (isMakefileAutomaticVar(line[i]) || std::isalnum(line[i]) ||
                        line[i] == '_') {
                        i++;
                        tokens.push_back(
                            {line.substr(start, i - start), TokenType::TYPE, start, i});
                        continue;
                    }
                }
            }
        ...
        // 运算符、括号和分隔符 （基于tree-sitter-make）
        if (isMakefileOperator(line[i]) || isMakefileBracket(line[i]) ||
            isMakefileDelimiter(line[i])) {
            tokens.push_back({std::string(1, line[i]), TokenType::OPERATOR, i, i + 1});
            i++;
            continue;
        }
...
std::vector<Token> SyntaxHighlighter::tokenizeSML(const std::string& line) {
    ...
    while (i < line.length()) {
        ...
        if (std::isalpha(c) || line[i] == '_' || line[i] == '\'') {
            ...
            // SML关键字
            static const std::unordered_set<std::string> sml_keywords = {
                "abstype",   "and",     "andalso",   "as",      "case",  "datatype",  "do",
                "else",      "end",     "exception", "fn",      "fun",   "handle",    "if",
                "in",        "infix",   "infixr",    "let",     "local", "nonfix",    "of",
                "op",        "open",    "orelse",    "raise",   "rec",   "then",      "type",
    ...
    while (i < line.length()) {
        ...
        if (std::isdigit(line[i]) ||
            ...
            if (line[i] == '0' && i + 1 < line.length() &&
                ...
                i += 2;
                while (i < line.length() &&
                       (std::isdigit(line[i]) || (line[i] >= 'a' && line[i] <= 'f') ||
                        (line[i] >= 'A' && line[i] <= 'F'))) {
                    i++;
                }
            }
            // 二进制
            else if (line[i] == '0' && i + 1 < line.length() &&
        ...
        if (std::isalpha(c) || line[i] == '_' || line[i] == '.') {
            ...
            // 常见的汇编指令
            static const std::unordered_set<std::string> asm_instructions = {
                // x86/x64指令
                "mov", "add", "sub", "mul", "div", "push", "pop", "call", "ret", "jmp", "je", "jne",
                "jz", "jnz", "jg", "jl", "jge", "jle", "cmp", "test", "and", "or", "xor", "not",
                "shl", "shr", "sar", "rol", "ror", "lea", "int", "nop", "hlt", "cli", "sti", "in",
                "out", "inc", "dec", "neg", "adc", "sbb", "imul", "idiv", "cbw", "cwd", "cdq",
                "cqo", "movsx", "movzx", "sete", "setne", "seta", "setae", "setb", "setbe", "setg",
                "setge", "setl", "setle", "cmov", "xchg", "bsf", "bsr", "bt", "btc", "btr", "bts",
                "cpuid", "rdtsc", "syscall", "sysenter", "sysexit",
                // RISC-V指令
                "addi", "slti", "sltiu", "andi", "ori", "xori", "slli", "srli", "srai", "add",
                "sub", "sll", "slt", "sltu", "xor", "srl", "sra", "or", "and", "fence", "ecall",
                "ebreak", "lbu", "lhu", "lb", "lh", "lw", "lwu", "ld", "sb", "sh", "sw", "sd",
                "jal", "jalr", "beq", "bne", "blt", "bge", "bltu", "bgeu", "lui", "auipc",
                // MIPS指令
                "addu", "addiu", "subu", "mult", "multu", "div", "divu", "mfhi", "mflo", "mthi",
                "mtlo", "sll", "srl", "sra", "sllv", "srlv", "srav", "jr", "jalr", "syscall",
                "break", "lb", "lbu", "lh", "lhu", "lw", "lwl", "lwr", "sb", "sh", "sw", "swl",
                "swr", "ll", "sc", "beq", "bne", "bgez", "bgtz", "blez", "bltz", "bgezal", "bltzal",
                "j", "jal"};

            // 寄存器
            static const std::unordered_set<std::string> asm_registers = {
                // x86/x64寄存器
                "rax", "rbx", "rcx", "rdx", "rsi", "rdi", "rbp", "rsp", "r8", "r9", "r10", "r11",
                "r12", "r13", "r14", "r15", "eax", "ebx", "ecx", "edx", "esi", "edi", "ebp", "esp",
                "ax", "bx", "cx", "dx", "si", "di", "bp", "sp", "al", "bl", "cl", "dl", "ah", "bh",
                "ch", "dh", "cs", "ds", "es", "fs", "gs", "ss", "rip", "eflags",
                // RISC-V寄存器
            ...
            // 伪指令
            static const std::unordered_set<std::string> asm_directives = {
                ".global", ".globl",   ".local",  ".extern",  ".data",  ".text", ".bss",
                ".rodata", ".align",   ".balign", ".p2align", ".byte",  ".word", ".long",
                ".quad",   ".ascii",   ".asciz",  ".string",  ".space", ".zero", ".comm",
                ".lcomm",  ".section", ".equ",    ".set",     ".macro", ".endm", ".include",
...
std::vector<Token> SyntaxHighlighter::tokenizeLLVMIR(const std::string& line) {
    std::vector<Token> tokens;
    size_t i = 0;
    ...
    while (i < line.length()) {
        ...
        if (std::isalpha(line[i]) || line[i] == '_' || line[i] == '%' || line[i] == '@') {
            ...
            // LLVM IR类型
            static const std::unordered_set<std::string> llvm_types = {
                "void",      "i1",    "i8",       "i16",     "i32",   "i64",
                "i128",      "float", "double",   "half",    "fp128", "x86_fp80",
                "ppc_fp128", "label", "metadata", "x86_mmx", "token"};

            // LLVM IR指令
            static const std::unordered_set<std::string> llvm_instructions = {"add",
                                                                              "sub",
                                                                              "mul",
                                                                              "udiv",
                                                                              "sdiv",
...
std::vector<Token> SyntaxHighlighter::tokenizeCommonLisp(const std::string& line) {
    ...
    while (i < line.length()) {
        ...
        if (line[i] == '#' && i + 1 < line.length() && line[i + 1] == '|') {
            ...
            i += 2;
            int comment_depth = 1;
            while (i < line.length() && comment_depth > 0) {
                if (line[i] == '|' && i + 1 < line.length() && line[i + 1] == '#') {
                    comment_depth--;
                    i += 2;
                } else if (line[i] == '#' && i + 1 < line.length() && line[i + 1] == '|') {
        ...
        if (line[i] == '\'' || line[i] == '`' ||
            (line[i] == ',' && i + 1 < line.length() && line[i + 1] == '@')) {
            if (line[i] == ',' && i + 1 < line.length() && line[i + 1] == '@') {
                tokens.push_back({",@", TokenType::OPERATOR, i, i + 2});
                i += 2;
            } else {
                tokens.push_back({std::string(1, line[i]), TokenType::OPERATOR, i, i + 1});
                i++;
        ...
        if (std::isalpha(c) || line[i] == '_' || line[i] == '-' || line[i] == '+' ||
            ...
            // Common Lisp关键字
            static const std::unordered_set<std::string> lisp_keywords = {
                "defun",
                "defmacro",
                "defvar",
                "defparameter",
