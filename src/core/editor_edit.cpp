...
void Editor::insertChar(char ch) {
    ...
    if (lsp_enabled_ && document_change_tracker_) {
        ...
        document_change_tracker_->recordInsert(
            static_cast<int>(cursor_row_), 
            static_cast<int>(cursor_col_), 
            new_text
        );
    }
#endif
...
void Editor::cut() {
    ...
    if (!selection_active_) {
        ...
        // 剪切选中内容
        content = getCurrentDocument()->getSelection(
    ...
    if (utils::Clipboard::copyToSystem(content)) {
        ...
        setStatusMessage(selection_active_ ? "Selection cut to clipboard" : "Line cut to clipboard");
    } else {
        // 如果系统剪贴板不可用，使用内部剪贴板
        getCurrentDocument()->setClipboard(content);
...
void Editor::copy() {
    ...
    if (utils::Clipboard::copyToSystem(content)) {
        ...
        // 同时保存到内部剪贴板（作为备份）
        getCurrentDocument()->setClipboard(content);
        setStatusMessage(selection_active_ ? "Selection copied to clipboard" : "Line copied to clipboard");
    } else {
        ...
        if (screen_height > 0) {
            ...
            if (cursor_screen_pos < 0 || cursor_screen_pos >= screen_height) {
                ...
                if (cursor_screen_pos < margin) {
                    view_offset_row_ = (cursor_row_ >= static_cast<size_t>(margin)) ? 
                        cursor_row_ - margin : 0;
