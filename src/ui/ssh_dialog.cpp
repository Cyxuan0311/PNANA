...
Element SSHDialog::render() {
    ...
    // 字段标签和输入框
    auto renderField = [&](const std::string& label, std::string& value, size_t field_idx, bool is_password = false) -> Element {
        ...
        field_elements.push_back(
            text(label + ": ") | color(colors.comment) | size(WIDTH, EQUAL, 15)
        );
        ...
        if (is_focused) {
            ...
            std::string cursor_char = cursor_pos < display_value.length() ? 
                                      display_value.substr(cursor_pos, 1) : " ";
            std::string after = cursor_pos < display_value.length() ? 
                                display_value.substr(cursor_pos + 1) : "";
            
            field_elements.push_back(
                hbox({
                    text(before) | color(colors.foreground),
                    text(cursor_char) | 
            ...
            field_elements.push_back(
                ...
                color(display_value.empty() ? colors.comment : colors.foreground) |
                dim
            );
        }
...
std::string* SSHDialog::getCurrentField() {
    switch (current_field_) {
        case 0: return &host_input_;
        case 1: return &user_input_;
        case 2: return &password_input_;
        case 3: return &key_path_input_;
        case 4: return &port_input_;
        case 5: return &remote_path_input_;
