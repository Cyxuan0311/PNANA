...
bool PluginManagerDialog::handleInput(ftxui::Event event) {
    if (!visible_ || !plugin_manager_) return false;
...
Element PluginManagerDialog::render() {
    ...
    content.push_back(
        hbox({
            text(" Plugin Manager ") | bold | color(colors.foreground),
            filler(),
    ...
    if (plugins_.empty()) {
        content.push_back(
            text("  No plugins found") | color(colors.comment) | center
        );
    } else {
    ...
    content.push_back(
        hbox({
            ...
            text(": Navigate  "),
            text("Space/Enter") | color(colors.function) | bold,
            text(": Toggle  "),
...
Element PluginManagerDialog::renderPluginItem(const plugins::PluginInfo& plugin, size_t /* index */, bool is_selected) {
    auto& colors = theme_.getColors();
    ...
    item_content.push_back(
        hbox({
            text("    "),
            text(desc) | color(colors.comment) | dim,
            text(author_info) | color(colors.comment) | dim,
            filler()
        })
    );
    
    return vbox(item_content) | (is_selected ? bgcolor(colors.selection) : bgcolor(colors.background));
}
...
void PluginManagerDialog::togglePlugin(size_t index) {
    if (index >= plugins_.size() || !plugin_manager_) return;
