...
CursorConfigDialog::CursorConfigDialog(Theme& theme)
    : theme_(theme),
      ...
      selected_option_(0),
      color_input_index_(1),
      rate_input_index_(2),
      smooth_cursor_index_(3),
      color_input_("255,255,255"),
      rate_input_("500") {
...
bool CursorConfigDialog::handleInput(ftxui::Event event) {
    ...
    if (selected_option_ == smooth_cursor_index_) {
        if (event == Event::Return || 
            ...
            (event.is_character() && (event.character() == " " || event.character() == "t" || event.character() == "T"))) {
...
Element CursorConfigDialog::render() {
    ...
    content.push_back(
        hbox({
            ...
            text(" Cursor Configuration ") | bold,
            filler()
    ...
    content.push_back(
        hbox({
            ...
            text("←→") | color(colors.function) | bold,
            text(": Change  "),
            text("Space/T") | color(colors.function) | bold,
            text(": Toggle"),
            filler()
...
Element CursorConfigDialog::renderStyleSelector() {
    ...
    for (size_t i = 0; i < style_options_.size(); ++i) {
        ...
        if (i > 0) style_elements.push_back(text("  "));
        
        if (is_current) {
            style_elements.push_back(
                text("[" + option + "]") | bold | color(colors.function) | bgcolor(colors.selection)
            );
        } else {
            style_elements.push_back(
                text(option) | color(is_selected ? colors.foreground : colors.comment)
            );
        }
    }
    ...
    return hbox({
        text("  "),
        (is_selected ? text("► ") | color(colors.function) : text("  ")),
...
Element CursorConfigDialog::renderColorSelector() {
    ...
    return hbox({
        ...
        text("█") | color(preview_color) | bgcolor(preview_color),
        text(" "),
...
Element CursorConfigDialog::renderRateSelector() {
    ...
    return hbox({
        ...
        text(rate_display) | 
            color(is_selected ? colors.foreground : colors.comment) |
            (is_selected ? bgcolor(colors.selection) : bgcolor(colors.background)),
        text(rate_desc) | color(colors.comment),
        filler()
    }) | (is_selected ? bgcolor(colors.selection) : bgcolor(colors.background));
}
...
Element CursorConfigDialog::renderSmoothCursorSelector() {
    ...
    return hbox({
        ...
        text(" (Space/T: toggle)") | color(colors.comment) | dim,
        filler()
    }) | (is_selected ? bgcolor(colors.selection) : bgcolor(colors.background));
}
...
void CursorConfigDialog::apply() {
    ...
    try {
        ...
        if (blink_rate_ < 0) blink_rate_ = 0;
    } catch (...) {
        blink_rate_ = 500;
    }
