...
pnana 支持通过配置文件自定义 LSP（Language Server Protocol）语言服务器。内置配置始终生效，`servers` 中的条目会**追加**到内置列表；若 `language_id` 与内置冲突，以代码中的内置配置为准。
...
| `servers` | array | `[]` | 额外服务器配置列表。追加到内置配置；冲突时以内置为准 |
