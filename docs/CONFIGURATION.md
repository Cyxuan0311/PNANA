...
| `show_welcome_logo` | boolean | `true` | 是否显示欢迎屏幕 Logo |
| `welcome_logo_top_margin` | number | `2` | Logo 距离顶部的空行数（仅在 `welcome_screen_flex` 为 `false` 时生效） |
| `welcome_screen_flex` | boolean | `true` | 是否启用 flex 布局（启用时欢迎屏幕垂直居中，`welcome_logo_top_margin` 无效） |
| `show_welcome_version` | boolean | `true` | 是否显示版本信息 |
| `show_welcome_start_hint` | boolean | `true` | 是否显示"Press i to start editing"提示 |
| `show_welcome_quick_start` | boolean | `true` | 是否显示快速开始快捷键（Ctrl+O/S/N/Q） |
| `show_welcome_features` | boolean | `true` | 是否显示功能介绍（Ctrl+F/G/T/Z） |
| `show_welcome_tips` | boolean | `true` | 是否显示提示信息（Tip + 主题提示 + 底部信息栏） |
