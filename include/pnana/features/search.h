...
struct SearchMatch {
    ...
    SearchMatch(size_t l, size_t c, size_t len)
...
public:
    ...
    // 替换
    bool replaceCurrentMatch(const std::string& replacement,
                            std::vector<std::string>& lines);
    size_t replaceAll(const std::string& replacement,
                     std::vector<std::string>& lines);
    
    // 获取匹配信息
    const SearchMatch* getCurrentMatch() const;
    size_t getCurrentMatchIndex() const { return current_match_index_; }
    size_t getTotalMatches() const { return matches_.size(); }
    const std::vector<SearchMatch>& getAllMatches() const { return matches_; }
...
private:
    ...
    // 内部搜索方法
    void searchLiteral(const std::string& pattern,
                      const std::vector<std::string>& lines);
    void searchRegex(const std::string& pattern,
                    const std::vector<std::string>& lines);
    void searchWholeWord(const std::string& pattern,
                        const std::vector<std::string>& lines);
};
