...
public:
    ...
    // 获取当前配置
    CursorStyle getCursorStyle() const { return cursor_style_; }
    std::string getCursorColor() const { return cursor_color_; }
    int getBlinkRate() const { return blink_rate_; }
    bool getSmoothCursor() const { return smooth_cursor_; }
    
    // 设置配置
    void setCursorStyle(CursorStyle style) { cursor_style_ = style; }
    void setCursorColor(const std::string& color) { cursor_color_ = color; }
    void setBlinkRate(int rate) { blink_rate_ = rate; }
    void setSmoothCursor(bool smooth) { smooth_cursor_ = smooth; }
    
    // 应用配置回调
    void setOnApply(std::function<void()> callback) { on_apply_ = callback; }
