#ifndef PNANA_UI_PACKAGE_DETAIL_DIALOG_H
#define PNANA_UI_PACKAGE_DETAIL_DIALOG_H

#include "features/package_manager/package_manager_base.h"
#include "ui/theme.h"
#include <ftxui/component/event.hpp>
#include <ftxui/dom/elements.hpp>
#include <memory>
#include <mutex>
#include <string>

namespace pnana {
namespace ui {

// 包详情弹窗
class PackageDetailDialog {
  public:
    explicit PackageDetailDialog(Theme& theme);
    ~PackageDetailDialog() = default;

    // 显示详情弹窗
    void show(const features::package_manager::Package& package,
              std::shared_ptr<features::package_manager::PackageManagerBase> manager);

    // 隐藏详情弹窗
    void hide();

    // 是否可见
    bool isVisible() const {
        return visible_;
    }

    // 处理输入
    bool handleInput(ftxui::Event event);

    // 渲染弹窗
    ftxui::Element render() const;

  private:
    Theme& theme_;
    bool visible_;
    features::package_manager::Package package_;
    std::shared_ptr<features::package_manager::PackageManagerBase> manager_;

    // 操作状态（线程安全）
    mutable std::mutex operation_mutex_;
    std::string operation_status_; // 操作状态消息
    bool operation_in_progress_;   // 操作是否进行中
    bool operation_success_;       // 操作是否成功（仅在完成时有效）

    // 渲染包信息
    ftxui::Element renderPackageInfo() const;

    // 渲染操作状态
    ftxui::Element renderOperationStatus() const;
};

} // namespace ui
} // namespace pnana

#endif // PNANA_UI_PACKAGE_DETAIL_DIALOG_H
