  ...
  private:
    ...
    inline json normalize_parameter(const std::string& name, const json& params) {
        if (params.type() == json::value_t::array) {
            ...
            if (mapping.find(name) == mapping.end()) {
                throw JsonRpcException(
                    invalid_params, "invalid parameter: procedure doesn't support named parameter");
            }
            json result;
            for (auto const& p : mapping[name]) {
                if (params.find(p) == params.end()) {
                    throw JsonRpcException(
                        invalid_params, "invalid parameter: missing named parameter \"" + p + "\"");
                }
        ...
        throw JsonRpcException(invalid_request,
                               "invalid request: params field must be an array, object");
    }
};
} // namespace jsonrpccxx
