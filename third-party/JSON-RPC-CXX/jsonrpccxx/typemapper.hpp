...
constexpr json::value_t GetType(type<void>) {
    return json::value_t::null;
}
constexpr json::value_t GetType(type<std::string>) {
    return json::value_t::string;
}
constexpr json::value_t GetType(type<bool>) {
    return json::value_t::boolean;
}
constexpr json::value_t GetType(type<float>) {
    return json::value_t::number_float;
}
constexpr json::value_t GetType(type<double>) {
    return json::value_t::number_float;
}
constexpr json::value_t GetType(type<long double>) {
    return json::value_t::number_float;
}
constexpr json::value_t GetType(type<short>) {
    return json::value_t::number_integer;
}
constexpr json::value_t GetType(type<int>) {
    return json::value_t::number_integer;
}
constexpr json::value_t GetType(type<long>) {
    return json::value_t::number_integer;
}
constexpr json::value_t GetType(type<long long>) {
    return json::value_t::number_integer;
}
constexpr json::value_t GetType(type<unsigned short>) {
    return json::value_t::number_unsigned;
}
constexpr json::value_t GetType(type<unsigned int>) {
    return json::value_t::number_unsigned;
}
constexpr json::value_t GetType(type<unsigned long>) {
    return json::value_t::number_unsigned;
}
constexpr json::value_t GetType(type<unsigned long long>) {
    return json::value_t::number_unsigned;
}
...
inline void check_param_type(size_t index, const json& x, json::value_t expectedType,
    ...
    if (expectedType == json::value_t::number_unsigned &&
        ...
        if (x.get<long long int>() < 0)
            throw JsonRpcException(invalid_params,
                                   "invalid parameter: must be " + type_name(expectedType) +
                                       ", but is " + type_name(x.type()),
                                   index);
        ...
        if (x.get<long long unsigned>() > (long long unsigned)std::numeric_limits<T>::max()) {
            throw JsonRpcException(
                invalid_params,
                "invalid parameter: exceeds value range of " + type_name(expectedType), index);
        }
    } else if ((x.type() == json::value_t::number_unsigned ||
                x.type() == json::value_t::number_integer) &&
               expectedType == json::value_t::number_float) {
...
MethodHandle createMethodHandle(std::function<ReturnType(ParamTypes...)> method,
    ...
    MethodHandle handle = [method](const json& params) -> json {
        ...
        (check_param_type<typename std::decay<ParamTypes>::type>(
             index, params[index], GetType(type<typename std::decay<ParamTypes>::type>())),
         ...);
...
NotificationHandle createNotificationHandle(std::function<void(ParamTypes...)> method,
                                            std::index_sequence<index...>) {
    NotificationHandle handle = [method](const json& params) -> void {
        size_t actualSize = params.size();
        size_t formalSize = sizeof...(ParamTypes);
        // TODO: add lenient mode for backwards compatible additional params
        // if ((!allow_unkown_params && actualSize != formalSize) || (allow_unkown_params &&
        // actualSize < formalSize)) {
        if (actualSize != formalSize) {
            throw JsonRpcException(invalid_params,
                                   "invalid parameter: expected " + std::to_string(formalSize) +
                                       " argument(s), but found " + std::to_string(actualSize));
...
NotificationHandle notificationHandle(void (T::*method)(ParamTypes...), T& instance) {
    std::function<void(ParamTypes...)> function = [&instance,
                                                   method](ParamTypes&&... params) -> void {
        return (instance.*method)(std::forward<ParamTypes>(params)...);
    };
