...
class JsonRpcServer {
  public:
    ...
    bool Add(const std::string& name, NotificationHandle callback,
             const NamedParamMapping& mapping = NAMED_PARAM_MAPPING) {
        if (name.rfind("rpc.", 0) == 0)
            return false;
        return dispatcher.Add(name, callback, mapping);
    ...
    std::string HandleRequest(const std::string& requestString) override {
        try {
            ...
            return json{
                {"id", nullptr},
                {"error",
                 {{"code", parse_error}, {"message", std::string("parse error: ") + e.what()}}},
                {"jsonrpc", "2.0"}}
                .dump();
        }
    }
    ...
    json HandleSingleRequest(json& request) {
        ...
        try {
            ...
            return json{
                {"id", id},
                {"error",
                 {{"code", internal_error}, {"message", std::string("internal server error")}}},
                {"jsonrpc", "2.0"}};
        }
    }
    ...
    json ProcessSingleRequest(json& request) {
        ...
        if (has_key(request, "params") &&
            ...
            throw JsonRpcException(
                invalid_request, "invalid request: params field must be an array, object or null");
        }
        ...
        if (!has_key(request, "id")) {
            ...
            return {{"jsonrpc", "2.0"},
                    {"id", request["id"]},
                    {"result", dispatcher.InvokeMethod(request["method"], request["params"])}};
        }
    }
};
} // namespace jsonrpccxx
