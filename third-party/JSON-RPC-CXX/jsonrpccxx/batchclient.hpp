...
class BatchRequest {
  public:
    ...
    BatchRequest& AddMethodCall(const id_type& id, const std::string& name,
    ...
    BatchRequest& AddNamedMethodCall(const id_type& id, const std::string& name,
    ...
    BatchRequest& AddNotificationCall(const std::string& name,
                                      const positional_parameter& params = {}) {
        call.push_back({{"method", name}, {"params", params}, {"jsonrpc", "2.0"}});
        return *this;
    }

    BatchRequest& AddNamedNotificationCall(const std::string& name,
                                           const named_parameter& params = {}) {
        call.push_back({{"method", name}, {"params", params}, {"jsonrpc", "2.0"}});
        return *this;
    }
...
class BatchResponse {
  public:
        ...
        if (results.find(id) != results.end()) {
            try {
                ...
                throw JsonRpcException(parse_error,
                                       "invalid return type: " + std::string(e.what()));
            }
    ...
    bool HasErrors() {
        return !errors.empty() || !nullIds.empty();
    }
    const std::vector<size_t> GetInvalidIndexes() {
        return nullIds;
    }
        ...
        try {
            ...
            if (!response.is_array()) {
                throw JsonRpcException(
                    parse_error, std::string("invalid JSON response from server: expected array"));
            }
