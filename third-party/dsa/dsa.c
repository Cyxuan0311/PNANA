...
// ASCII详细字符集（从暗到亮）
static const char ASCII_DETAILED[] =
...
// 获取24位真彩色代码
void get_truecolor_code(unsigned char r, unsigned char g, unsigned char b, char* buffer,
                        size_t buffer_size) {
...
void get_color_code(unsigned char r, unsigned char g, unsigned char b, int color_mode, char* buffer,
                    size_t buffer_size) {
    ...
    for (int i = 0; i < new_height; i++) {
        for (int j = 0; j < effective_width; j++) {
            ...
            if (resolution_scale > 1.5f) {
                ...
                // 确保索引有效
                if (idx11 >= 0 && idx11 < x * y * n && idx12 >= 0 && idx12 < x * y * n &&
                    idx21 >= 0 && idx21 < x * y * n && idx22 >= 0 && idx22 < x * y * n) {
                    // 双线性插值计算
                    unsigned char r1 = (unsigned char)(data[idx11] * (1 - fx) + data[idx12] * fx);
                    unsigned char g1 =
                        (unsigned char)(data[idx11 + 1] * (1 - fx) + data[idx12 + 1] * fx);
                    unsigned char b1 =
                        (unsigned char)(data[idx11 + 2] * (1 - fx) + data[idx12 + 2] * fx);

                    unsigned char r2 = (unsigned char)(data[idx21] * (1 - fx) + data[idx22] * fx);
                    unsigned char g2 =
                        (unsigned char)(data[idx21 + 1] * (1 - fx) + data[idx22 + 1] * fx);
                    unsigned char b2 =
                        (unsigned char)(data[idx21 + 2] * (1 - fx) + data[idx22 + 2] * fx);
